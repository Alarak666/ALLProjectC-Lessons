        ...
        public void InfoEmployee()
        {
            try
            {
                ...
                SqlCommand command = new SqlCommand("SELECT ID" +
                    ",CONCAT(MName,' ', SUBSTRING(FName,1,1),'. ',SUBSTRING(LName,1,1),'.') AS 'FIO' FROM Employees", connections);
                SqlDataReader rdr;
            ...
            try
            {
                if (column == 0)
                {
                    ...
                    query = @"Select ID_Place from stock
                                    join product on product.id_product = stock.id_product 
                                    where product.id_product =  " + mproduct + " and Date = " +
                                    "(Select min(Date) from stock" +
                                    " where id_product = " + mproduct + ")";
