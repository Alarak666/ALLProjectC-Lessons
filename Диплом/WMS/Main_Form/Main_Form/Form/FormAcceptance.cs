...
namespace Main_Form
{
 ...
 ";
        ...
        {
            connections.Open();
            string query = @"SELECT AcceptanceOfGoods.ID_Acceptance
							...
							Where ID_Acceptance <> all (
							Select ID_Acceptance from PlaceOfProduct
							)
							";
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ID_Acceptance.Text = dataGridView1.Rows[e.RowIndex].Cells[0].Value.ToString();
                Type_Pallet.Text = dataGridView1.Rows[e.RowIndex].Cells[1].Value.ToString();
                ID_Product.Text = dataGridView1.Rows[e.RowIndex].Cells[2].Value.ToString();
                ID_Employyer.Text = dataGridView1.Rows[e.RowIndex].Cells[3].Value.ToString();
                _Width.Text = dataGridView1.Rows[e.RowIndex].Cells[4].Value.ToString();
                _Height.Text = dataGridView1.Rows[e.RowIndex].Cells[5].Value.ToString();
                _Lenght.Text = dataGridView1.Rows[e.RowIndex].Cells[6].Value.ToString();
                _Weight.Text = dataGridView1.Rows[e.RowIndex].Cells[7].Value.ToString();
                Qty.Text = dataGridView1.Rows[e.RowIndex].Cells[8].Value.ToString();
                DateAcceptance.Text = dataGridView1.Rows[e.RowIndex].Cells[9].Value.ToString();
                Date_Expiration.Text = dataGridView1.Rows[e.RowIndex].Cells[10].Value.ToString();
                ID_Pallet.Text = dataGridView1.Rows[e.RowIndex].Cells[11].Value.ToString();
                ID_Supply.Text = dataGridView1.Rows[e.RowIndex].Cells[12].Value.ToString();
            }
        }
     ...
     VALUES
        ...
        bool ID_Acceptance_bool()
        {
            int data;
            connections.Open();
            string query = @"Select ID_acceptance from AcceptanceofGoods WHERE ID_acceptance = " + ID_Acceptance.Text + "";
            SqlCommand command = new SqlCommand(query, connections);
            SqlDataReader rdr;
            rdr = command.ExecuteReader();
            while (rdr.Read())
            {
                data = Convert.ToInt32(rdr["ID_acceptance"].ToString());
                if(data == Convert.ToInt32(ID_Acceptance.Text))
                {
                    connections.Close();
                    return false;
                }
